...
class SquidCog(commands.GroupCog, group_name="squid"):
    ...
    @app_commands.command(name="redlight", description="Control Red Light Green Light game turns and doll signals")
    @app_commands.describe(action="Game phase to trigger")
    async def redlight(self, interaction: discord.Interaction, action: Literal["start", "green", "red"]):
        try:
            await interaction.response.defer()
        except discord.NotFound:
            return
        ...
        if action == "start":
            ...
            embed = build_red_light_embed("GREEN", 100, {})
            if audio_bytes:
                await self.audio_deliverer.deliver(interaction, audio_bytes, "redlight_start.wav", embed=embed)
            else:
                await interaction.followup.send(embed=embed)
        elif action == "green":
            ...
            if self.synthesizer:
                try:
                    audio_bytes = await self.synthesizer.synthesize(GuardVoiceLines.green_light_korean(), DOLL_PROFILE)
                except Exception:
                    audio_bytes = await self.synthesizer.synthesize(GuardVoiceLines.green_light_english(), GUARD_PROFILE)
            game = self.squid_service._active_games.get(guild_id, {})
            embed = build_red_light_embed("GREEN", game.get("target", 100), game.get("progress", {}))
            if audio_bytes:
                await self.audio_deliverer.deliver(interaction, audio_bytes, "doll_green.wav", embed=embed)
            else:
                await interaction.followup.send(embed=embed)
        elif action == "red":
            self.squid_service.set_light(guild_id, "RED")
            if self.synthesizer:
                audio_bytes = await self.synthesizer.synthesize(GuardVoiceLines.red_light(), GUARD_PROFILE)
            game = self.squid_service._active_games.get(guild_id, {})
            embed = build_red_light_embed("RED", game.get("target", 100), game.get("progress", {}))
            if audio_bytes:
                await self.audio_deliverer.deliver(interaction, audio_bytes, "doll_red.wav", embed=embed)
            else:
                await interaction.followup.send(embed=embed)
